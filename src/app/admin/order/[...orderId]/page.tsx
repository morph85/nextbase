...
import OrderConstants from "@/app/admin/order/_sub/constants"
...
const OrderDetail = (props: any) => {
  ...
  const keyinfos = OrderConstants.Form.KEYS
  ...
  const [attrs, setAttrs] = useState(OrderConstants.Form.ATTRS)
  ...
  const [rules] = useState({
    ...
    order: [{ required: true, message: ErrorConst.REQUIRED }],
  })
  ...
  const getData = useCallback(async () => {
    ...
    let response: any = {
      ...
      data: {
        name: "order-new-0",
        description: "Order New 0 Description",
        price: 1.23,
        sort: 2,
        disabled: true,
  ...
  return (
    <BasePage className="w-full min-h-[50vw]">
      <div className="w-full p-4 py-8">
        <AdminHeader title={`Order (ID: ${params.orderId})`}>
